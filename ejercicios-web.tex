...
Para la entrega de prácticas incrementales se utilizarán repositorios git públicos alojados en el GitLab de la EIF. Para cada práctica entregable los profesores abrirán un repositorio público en la carpeta de plantillas del curso actual, en el proyecto CursosWeb~\footnote{\url{https://gitlab.eif.urjc.es/CursosWeb}}, con un nombre que la identifique, similar al que aparece en el programa de la asignatura para el ejercicio correspondiente. En el caso de las miniprácticas, el nombre comenzará por ``Mini--'', seguido de un número (el de orden de entrega), y el identificador del ejercicio. Este repositorio incluirá un fichero README.md, con el enunciado de la práctica, y cualquier otro material que los profesores estimen conveniente.

Cada alumno dispone de una cuenta en el GitLab de la EIF, que usará a efectos de entrega de prácticas. Si el alumno lo desea, puede usar la misma cuenta en el GitLab de la EIF para otros fines, además de para la entrega de prácticas.

Para trabajar en una práctica, los alumnos comenzarán por realizar una copia (fork) de cada uno de estos repositorios. Esto se realiza en el GitLab de la EIF, visitando (tras haberse autenticado con su cuenta de usuario) el repositorio con la práctica, y pulsando sobre la opción de realizar un fork. Una vez esto se haya hecho, el alumno tendrá un fork del repositorio en su cuenta, con los mismos contenidos que el repositorio original de la práctica. Visitando este nuevo repositorio, el alumno podrá conocer la url para clonarlo, con lo que podrá realizar su clon (copia) local, usando la orden \verb|git clone|.
...
Conviene visitar el repositorio propio en el GitLab de la EIF, para comprobar que efectivamente los cambios realizados en la copia local se han propagado adecuadamente a él, tras haber invocado \verb|git push|.
...
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/mini-1-acortadora}
...
\begin{itemize}
    ...
    \item \textbf{Recurso ``/stats/alias'' (GET):} Devuelve una página HTML que muestra el número de veces que se ha accedido (redirigido) a la URL acortada correspondiente a ese alias. Si el alias no existe, se mostrará un mensaje de error de cliente (4XX).
\end{itemize}
...
\begin{itemize}
    ...
    \item Si alguien accede a \texttt{http://localhost:1234/miweb}, la aplicación redirige a la URL actualizada. Internamente se incrementará un contador de accesos para ese alias.
    \item Si alguien accede a \texttt{http://localhost:1234/stats/miweb}, la aplicación mostrará cuántas veces se ha redirigido ese alias.
\end{itemize}
...
\begin{itemize}
    ...
    \item Implementar ``GET /stats/alias'' para mostrar el número de accesos a cada URL acortada.
    \item Manejar las condiciones de error y realizar el resto de la funcionalidad.
\end{itemize}

\textbf{Comentario:} Se recomienda el uso de un diccionario para almacenar las URLs reales y los alias. La clave será el alias y el valor será la URL real. También será necesario almacenar de forma persistente el número de accesos (redirecciones) de cada alias.

%%---------------------------------------------------------------------
%%---------------------------------------------------------------------
\subsection{Minipráctica 2 (entrega voluntaria)}
\label{subsec:minipractica-2-2025}

\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/mini-2-acortadora}
...
\begin{itemize}
  ...
  \item Se añadirá un recurso \textbf{``/stats/alias''} (GET) que mostrará el número de veces que se ha accedido/redirigido a la URL acortada correspondiente a ese alias.

  \item Se añadirá un recurso \textbf{``/randomredirect''} (GET) que redirigirá a una de las URLs acortadas elegida aleatoriamente de entre todas las existentes. Si no hay URLs acortadas, mostrará un mensaje de error.

  \item Toda la información se almacenará en una base de datos (usando los modelos de Django), de forma que aunque la aplicación sea rearrancada, las URLs acortadas y los contadores de accesos sigan funcionando adecuadamente. Se recomienda guardar en la tabla de URLs un campo booleano que determine si el alias es aleatorio o no, y un campo contador de accesos.

  \item Se utilizarán plantillas para separar el código Python del HTML.
\end{itemize}
...
%%-----------------------------------------------------------------------------
%%-----------------------------------------------------------------------------
\subsection{Servidor web con carrito de la compra}
\label{subsec:servidor-carrito}

\textbf{Enunciado:}

Vamos a construir una aplicación servidora HTTP capaz de manejar usuarios, cada uno con su carrito de la compra, de forma sencilla y sin usar cookies. Para ello, haremos que la aplicación interprete parte de los nombres de recurso que le pidan como el identificador de usuario. Por ejemplo, podemos usar el formato \verb|{recurso}/{id}|, o \verb|{recurso}?{id}|.

Para que la aplicación funcione, cuando le pidan un recurso que no tiene identificador de usuario, asignará uno, y devovlerá todas sus páginas ``marcadas'' con ese identificador. Para ``marcarlas'', hará que todos los enlaces (por ejemplo, en elementos \verb|<a href>|) incluyan ese identificador, y que en todos los formularios este identificador esté como campo oculto. De esta manera, todas las peticiciones que el navegador envíe a nuestra aplicación, bien porque el usuario ha pulsado sobre un enlace, bien porque ha rellenado un formulario, le llegarán con el identificador, bien en el nombre del recurso, bien como parte de la query string que se envía con los formularios.

El carrito de cada usuario se puede simular con un diccionario en memoria, que tenga una clave para cada identificador de usuario, y como valor el objeto o los objetos que el usuario haya marcado para el carrito.

Cada página de la aplicación puede devolver al menos un enlace a una ``página de objeto''. En esa página, habrá un botón para poder añadir el objeto al carrito de la compra del usuario que lo pulse. Los objetos pueden estar indentificados por números. La página de cada objeto será por tanto un recurso que incluya ese identificador de objeto. Cuando una página (por ejemplo la página principal) incluya el enlace a un objeto, incluirá ese enlace. En una aplicación real, estos números indentificadores de objeto estarían en una base de datos, pero aún no se pide implementar esa parte: por ahora cuando queramos generar un objeto, simplemente generaremos un número aleatorio.

Además asegúrate que tu programa:

\begin{itemize}
\item Permite añadir al carrito de un usuario cualquier número de objetos.
\item Muestre si se hace un GET al recurso \texttt{/carrito} el contenido del carrito, un botón para comprar el carrito, y un enlace para volver a la página principal.
\item Si se pulsa el botón para comprar el carrito, se hace un POST al recurso \texttt{/comprar}, que devolverá un formulario que pedirá un número de tarjeta de crédito, y tendrá un botón para enviar dicho número, y un enlace para volver a la página principal.
\item Si se pulsa el botón para enviar el número de tarjeta de crédito, se debe hacer un POST al recurso \texttt{/comprar}, que devolverá un mensaje de confirmación, y un enlace para volver a la página principal.
\item Todas las páginas mostarán el valor de las cabeceras \texttt{Accept-Language} y \texttt{User-Agent}, si estas cabeceras vienen en la petición HTTP.
\item Todas las respuestas de tu servidor incluirán una cabecera Server con el valor \texttt{Servidor Sencillo 0.0}.
\end{itemize}

\textbf{Solución parcial:}

El programa \texttt{servidor.py} es la versión más simple, mientras que \texttt{servidor2.py} es una versión un poco más completa, que usa enlaces de la forma \texttt{/objetos/\{id\}} para acceder a los objetos, y tiene un formulario para añadir objetos solo en las páginas de los objetos. Ambos han sido hechos parcialmente con la ayuda de un asistente de IA.

Carpeta de código: \texttt{Web-Spec/servidor-carrito}

%%-----------------------------------------------------------------------------
%%-----------------------------------------------------------------------------
\subsection{Servidor web con carrito de la compra (cookies)}
\label{subsec:servidor-carrito-cookies}

\textbf{Enunciado:}

Realiza una aplicación web que permita manejar un carrito de la compra para cada usuario, tal y como se explica en el ejercicio ``Servidor web con carrito de la compra'' (ejercicio~\ref{subsec:servidor-carrito}), usando cookies en lugar de urls anotadas y campos ocultos en formularios. Además:

\begin{itemize}
\item La aplicación mantendrá en un diccionario un listado de usuarios reconocidos, y sus contraseñas
\item Si un navegador hace una petición a cualquier recurso de la aplicación, sin enviar una cabecera de cookie de sesión válida, el navegador recibirá como respuesta una página HTML con un formulario para escribir un identificador de usuario y la contraseña correspondiente. Si el usuario los escribe correctamente, según el listado de usuarios reconocidos, se abrirá una nueva sesión, enviando al navegador la cookie correspondiente. Si no, volverá a enviarle al navegador el formulario de entrada.
\item Cuando se haya establecido una sesión, el usuario podrá pulsar en un botón para terminar al sesion y ``salir'' de la aplicación.
\end{itemize}

Por lo tanto, solo cuando el usuario ``ha entrado'' en su cuenta podrá aañdir objetos a su carrito de la compra, y comprarlos.

\textbf{Solución parcial:}

El programa \texttt{servidor.py} es una versión simple, hecha con la ayuda de un asistente de IA.

Carpeta de código: \texttt{Web-Spec/servidor-cookies}
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2023-2024/contador-simple-rearranques}
...
%%-----------------------------------------------------------------------------
%%-----------------------------------------------------------------------------
\subsection{Traza de historiales de navegación por terceras partes (implementación)}
\label{subsec:navegacion-terceras-partes-impl}

Implementa un programa trazador, que funcione de forma similar a la descrita en el ejercicio ``Traza de historiales de navegación por terceras partes'' (ejercicio~\ref{subsec:navegacion-terceras-partes}).

El programa trazador se llamará \texttt{trazador.py}, y cuando sea lanzado admitirá un argumento, que será el puerto TCP en el que quedará esperando peticiones HTTP. Por ejemplo, si se lanza de la siguiente forma, escuchará peticiones en el puerto 8002:

\begin{verbatim}
python3 trazador.py 8002
\end{verbatim}

Este programa mantendrá en memoria una lista con los accesos que se vayan realizando a los sitios que monitorice. Para cada acceso, anotará:

\begin{itemize}
\item La fecha y hora del acceso.
\item El recurso solicitado.
\item El sitio web donde ha tenido lugar el acceso.
\item Un identificador único de navegador.
\item La cabecera de identificación HTTP del navegador que realizó el acceso.
\end{itemize}

En cuanto al identificador único de navegador, cada navegador recibirá uno, pero éste deberá ser el mismo cuando ese navegador acceda a los distintos sitios monitorizados. De esta forma se podrá trazar la historia de navegación de cada navegador en todos los sitios monitorizados.

Para poder trazar los acceso a los sitios monitorizados, \texttt{trazador.py} utilizará recursos con el prefijo \texttt{/rec/}.

Además, \texttt{trazador.py} permitirá registrarse y autenticarse. Para registrarse y autenticarse, se ofrecerá un formulario en la página principal ``/'', donde un usuario cualquiera podrá crearse una cuenta o, si ya la tiene, usarla para autenticarse. Para registrarse se pedirá una dirección de correo y una contraseña. Para autenticarse se pedirá de nuevo una dirección de correo y una contraseña. Una vez se hayan autenticado, los usuarios podrán también cerrar la sesión, con lo que si vuelven al recurso ``/'', tendrían que volver a autenticase.

Cuando un usuario se haya autenticado, el recurso principal le mosrtrará en formato tabla el histórico de accesos a todos los sitios monitorizados, como una tabla única, ordenada por fechas, con todos los datos recogidos para cada acceso. Si para alguno de esos navegadores se conoce la dirección de correo del usuario, gracias al mecanismo de registro, se mostrará también.

Además, esa tabla tendrá un enlace a un recurso donde el usuario autenticado se podrá descargar la misma información de la tabla en formato JSON.

Si un navegador, autenticado o no, accede a un recurso que no tenga el prefijo \texttt{/rec/}, que no sea el recurso principal (``/''), o que no sea el recurso que se use para cerrar la sesión, se le mostrará una página en blanco.

Para que el trazado de los sitios monitorizados funcione, habrá que diseñar un esquema de recursos que se servirán con el prefijo \texttt{/rec/}. Este esquema se describirá en el fichero README.md. Ahí se explicará cómo se identificará, a partir del recurso que se pida, el sitio web y el recurso que se está accediendo. Tambień se explicará el nombre de la cookie utilizada para saber cuál es el navegador que está realizando el acceso.

Para poder probar el esquema, se incluirá en el repositorio con la solución un directorio \texttt{web}, que tendrá al menos tres subdirectorios (\texttt{serv1}, \texttt{serv2} y \texttt{serv3}). Cada uno de esos subdirectorios tendrá al menos dos páginas, \texttt{hola.html} y \texttt{adios.html}. Estas páginas incluirán los recursos necesarios servidos por \texttt{trazador.py}, para que los accesos puedan ser trazados. Estos recursos supondrán que \texttt{trazador.py} espera peticiones HTTP en el puerto 8002 de 127.0.0.1 (localhost), como se ha indicado.

La prueba de la práctica se realizará lanzando un servidor en cada directorio \texttt{serv1}, \texttt{serv2} y \texttt{serv3}, cada uno en un puerto diferente. Por ejemplo, para \texttt{serv1}:
...
Y lanzando a continuación \texttt{trazador.py} en el puerto 8002, como se ha indicado.

Uan vez esté hecho esto, con varios navegadores (simulándolos, si es preciso, con ventanas de navegación en modo privado), se visitarán los servidores monitorizados, y se crearán cuentas para acceder a los datos del trazador, para probar la funcionalidad indicada.

\textbf{Solución parcial:}

El programa \texttt{trazador.py} es una versión simple, hecha con la ayuda de un asistente de IA.

Carpeta de código: \texttt{Python-Web/trazador}


%%-----------------------------------------------------------------------------
%%-----------------------------------------------------------------------------
\subsection{Protección frente a trackers en navegadores}
\label{subsec:trackers-navegadores}

Explora la protección frente a trackers (rastreadores) en Firefox. Visita varias páginas web, explora los trackers bloqueados, compara con lo que ves en las herrameientas de desarrollador con respecto a cookies. Entiende las implicaciones de la configuración sobre rastreadores de Firefox.
...
\label{subsec:calculadoras}

\textbf{Enunciado:}

Crea una aplicación web que permita crear calculadoras de varios tipos. Los usuarios  de la aplicación podrán crear calculadoras mediante un formulario que se obtiene en el recurso principal del la aplicación.  Cada calculadora tendrá un identificador único, decidido por la aplicación cuando se crea la calculadora.

Tendremos calculadoras de dos tipos:

\begin{itemize}
\item Calculadoras de IVA. Calcula el IVA de la cantidad que le demos
\item Calculadoras de sumas. Suma las dos cantidades que le demos.
\end{itemize}

Para crear una calculadora la aplicación ofrecerá un formulario con
un botón. Cada calculadora tendrá su propio recurso. Cuando se invoque ese recurso, devolverá un formulario para que el usuario indique los datos para calcular. Tras ponerlos y darle a enviar,  el usuario verá la cantidad resultante, y de nuevo el formulario de la calculadora.

En una segunda fase, añade la siguiente funcionalidad:

\begin{itemize}
\item El recurso principal de la aplicación mostrará un listado de las calculadoras que se han creado. Cada elemento del listado será un enlace a la calculadora correspondiente, y una declaración de su tipo.
\item Se usarán cookies para que sólo el navegador que creó una calculadora pueda acceder a ella. Si un navegador que no creó una calculadora trata de acceder a ella, recibirá un error de recurso no autorizado.
\item Se creará un mecanismo para poder ``transferir'' el permiso de acceso a una calculadora a otro navegador. Para ello, se usará algún mecanismo por el cual el segundo navegador reciba una URL a la que pueda acceder para recibir la cookie que permita el acceso a las calculadoras que creó el primer navegador (puede ser directamente una URL, o un QR, por ejemplo).
\end{itemize}

En una tercera fase, añade la funcionalidad que quieras.

Utiliza un fichero \texttt{spec.md} para definir la especificación de la aplicación. Este fichero tendrá que tener secciones para:

\begin{itemize}
\item Requisitos funcionales (genéricos)
\item Requisitos no funcionales
\item Estado (datos que se mantienen por la aplicación entre invocaciones HTTP)
\item Inicialización: acciones a realizar cuando arranque la aplicación
\item Recursos: descripción de todos los recursos que atiende la aplicación, las operaciones para cada uno de ellos, la información de la petición que se atiende para cada uno de ellos, y la información que se enviará en la respuesta correspondiente. 
\end{itemize}

Utiliza un fichero \texttt{entrega.md} para indicar, brevemente:

\begin{itemize}
\item Hasta qué fase has realizado.
\item Qué parte de lo especificado has realizado en cada fase.
\end{itemize}

\textbf{Solución parcial:}

El fichero \texttt{spec.md} presenta una posible especificación de la primera parte del enunciado. A partir de ella se ha generado automáticamente el programa \texttt{calculadoras.py}, y las pruebas (tests) del programa \texttt{test\_calculadoras.py}.

Carpeta de código: \texttt{Python-Web/calculadoras}
...
%%-----------------------------------------------------------------------------
\subsection{Lista de la compra con especificación}
\label{subsec:lista-compra-spec}

\textbf{Enunciado:}

Vamos a diseñar una aplicación HTTP que mantiene una lista de la compra. Cada elem ento de la lista tendrá un nombre (por ejemplo, "leche") y una cantidad (por ejempl o 5). La aplicación me permitirá ver los elementos de la lista y la cantidad de cad a elemento. También me permitirá incluir nuevos elementos en la lista, darles una cantidad, y cambiar la cantidad de un elemento que ya tenía en la lista.

Puede haber elementos en la lista de la compra con valor 0, si se encuentra que es útil por algún motivo.

Diseñar una especificación para esta aplicación, indicando cuál será el estado que tiene que mantener, y cuál será su API HTTP. En la medida de lo posible, realizar esta API siguiendo los principios REST.

Pueden realizarse dos versiones de la especificación: una, utilizando las operaciones GET y PUT, que supone que se va a utilizar desde una aplicación en la que se pueda hacer un PUT de HTTP; y otra, utilizando GET y POST, las operaciones que se podrían hacer si el navegador recibe solo HTML ``puro'', sin JavaScript. Para la versión con PUT se puede usar HTMX como forma de crear formularios que puedan enviar un PUT a la aplicación.

Cuando se hayan hecho las especificaciones, implementar las aplicaciones correspondientes. Luego, probar estas aplicaciones, y ver las interacciones entre el navegador y la aplicación con las herramientas del desarrollador del navegador.

\textbf{Solución parcial:}

Los ficheros \texttt{spec.md} y \texttt{spec2.md} presentan una posible especificación de las dos versiones indicadas en el enunciado. A partir de ellas se han generado automáticamente los programas \texttt{compra.py} y \texttt{compra2.py}, junto con \texttt{plantillas.py} y \texttt{plantillas2.py}, respectivamente.

Carpeta de código: \texttt{Web-Spec/compra}
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2022-2023/calculadora-simple-rest-django}
...
%%-----------------------------------------------------------------------------
%%-----------------------------------------------------------------------------
\subsection{Sitio web para gestionar issues de GitLab}
\label{subsec:gitlab-issues}

\textbf{Enunciado:}

Vamos a construir, por fases, un sitio web para acceder a tickets (issues) del GitLab de la EIF.

Crea un repositorio en el GitLab de la EIF, y crea algún ticket (issues) en él. Comenta también alguno de los tickets. Familiarízate con los procedimientos para hacerlo.

Utiliza \texttt{curl} para acceder a los tickets de tu repositorio. Por ejemplo (ojo, todo en una línea):

\begin{verbatim}
curl -s "https://gitlab.eif.urjc.es/api/v4/projects/
cursoprogram%2Fbifurca-repositorio/issues?state=all&per_page=100" | jq '.'
\end{verbatim}

Escribe un script de Python que haga lo mismo, pero mostrando en lugar del documento JSON que te descargas de la API, el listado de tickets, con solo su título. Para hacerlo, crea una función (puede ser un método de una clase) que se encargue de la descarga de la lista de tickets. Llámalo \texttt{issues.py}.

Escribe un servidor web que que muestre en su página principal un formulario, en el que se puede poner el nombre de un repositorio. Cuando se ponga, mostrará el listado de los issues de ese repositorio.  Usa la función (o clase) definida en \texttt{issues.py} para acceder a la lista de issues del GitLab de EIF. Llámalo \texttt{server.py}.

Crea un programa con una clase que tenga una función para listar los issues de un proyecto en el GitLab de la EIF, y otra para crear un nuevo issue en ese mismo repositorio. El programa principal instanciará esa clase, y aceptará argumentos para listar issues, y para crear uno nuevo. Lee un fichero \texttt{.env} para leer el token de GitLab de él. Llámalo \texttt{gitlab.py}

Crea un servidor HTTP que muestre, si se solicita su recurso principal, un formulario para elegir un repositorio. Una vez recibido el formulario elegido, el servidor enviará una redirección a un recurso que se llame igual que el repositorio elegido. Además, si se solicita un recurso con nombre de repositorio, devolverá el listado de issues del repositorio, y un formulario donde podrá ponerse un nuevo issue (mediante POST sobre ese mismo repositorio). Para listar y poner issues, usa la clase de \texttt{gitlab.py}. Llámalo \texttt{server2.py}.

Utiliza \texttt{curl} para traducir un texto. Por ejemplo:
...
Crea un servidor HTTP como \texttt{server2.py}, pero que cuando se pone un issue, ofrezca la opción de traducir el texto del issue al inglés, y si se selecciona esa opción, se cree el issue en GitLab en inglés.  Llámalo \texttt{server3.py}.

Finalmente, crea un servidor para manejar issues en el GitLab de la EIF. Ese servidor ha de permitir listar tickets (issues) de cualquier repositorio, ver cualquiera de ellos, en su idioma original, traducidos al inglés o traducidos al español, y poner tickets en cualquier repositorio, en el idioma del usuario, o traducidos al inglés.

Opcionalmente, se puede utilizar la API de traducción para traducir la propia interfaz de usuario del servidor web (las páginas HTML que ve el usuario).

Cada recurso que se pueda invocar del servidor admitirá la opción de recibir la respuesta en formato JSON si en la petición se incluye la cabecera ``Content-Type'' con valor \texttt{application/json}.

Además, para evitar llamadas innecesarias a la API de traducción, se guardará todo lo traducido en una cache en memoria, de forma que si algo ya fue traducido, no se volverá a traducir.

\textbf{Referencias:}

Documentación sobre la API de GitLab: \\
\url{https://docs.gitlab.com/api/rest/}

Documentación sobre la API de Langbly: \\
\url{https://langbly.com/docs/}

\textbf{Solución parcial:}

Los programas \texttt{issues.py}, \texttt{gitlab.py}, \texttt{translator.py}, \texttt{server.py} y \texttt{server2.py} son versiones relativamente simples, hecha con la ayuda de un asistente de IA (los dos servidores usan \texttt{templates.py} y \texttt{templates2.py}, respectivamente).

Carpeta de código: \texttt{Python-Web/gitlab}
...
%%-----------------------------------------------------------------------------
\subsection{Framework sencillo para aplicaciones web}
\label{subsec:framework-sencillo}

\textbf{Enunciado:}

Vamos a construir un framework sencillo que nos permita construir aplicaciones web con más facilidad. Para ello, vamos a centrarnos en los elementos que normalmente tenemos en cualquier aplicación web:

\begin{itemize}
\item Modelo: Gestiona el estado y los datos, que se almacenarán persistentemente. Proporcionará mecanismos para manejar este estado de forma lo más genérica posible, de forma que sirva para diferentes lógicas de aplicación.
\item Vista: Presenta la información (páginas HTML, formularios, etc.) al usuario, y atiende a sus peticiones (query strings en POST, por ejemplo). La lógica de aplicación específica se dejará para que la proporcione quien desarrolla la aplicación.
\item Controlador: Maneja las peticiones HTTP y coordina modelo y vista. Se encargará de toda la inicialización para poder atender peticiones HTTP, y de proporcionar mecanismos para llamar a la lógica de aplicación según las peticiones HTTP recibidas, para acceder al estado (vía el modelo) cuando sea preciso.
\end{itemize}

Realiza una especificación del framework, y luego una implementación de esa especificación. Pruébala con alguna aplicación.

Añade al framework la siguiente funcionalidad:

\begin{itemize}
\item Gestión de cookies. El framework debe proporcionar un módulo, \texttt{cookies.py} que proporcione funcionalidad para crear cookies, enviáselas al navegador en respuestas HTTP, y extraer cookies que puedan venir en una petición HTTP.
\item Gestion de sesiones. El framework debe proporcionar un módulo \texttt{sessions.py} que proporcione funcionalidad para mantener sesiones utilizando cookies. El módulo reconocerá cuándo se reciben peticiones HTTP de un navegador del que se han recibido peticiones en el pasado. Se dará un identificador a cada navegador (sesión), y se mostrará en un log el identificador del navegador cada vez que se reciba una petición. Este módulo utilizará la funcionadlidad de \texttt{cookies.py}.
\item Gestión de almacenamiento para sesiones. El framework debe proporcionar un módulo \texttt{session\_storage.py} que proporcione funcionalidad para almacenar datos relacionados con un identificador de sesión. En particular, esta funcionalidad permitirá almacenar datos (claves y valores) asociados a un identificador de sesión, y recuperar un cierto dato dada una clave y un identificador de sesión.
\end{itemize}


Utiliza la implementación para construir (primero, escribiendo su especificación) una aplicación que use toda esta funcionalidad. Puede ser, por ejemplo, una lista de cosas por hacer, donde la lista sea ``propia'' para cada navegador que utilice la aplicación web.

Procura construir el framework de forma que las aplicaciones que lo usen se puedan escribir con la mínima cantidad de código posible. Asegúrate de que tanto el framework como la aplicación funcionan correctamente. Incluye un fichero README.md que describa cómo funciona el framework, y cómo funciona la aplicación.

\textbf{Solución parcial:}

Los ficheros \texttt{spec.md} y \texttt{spec2.md} son especificaciones parciales para lo que se pide en este ejercicio. El resto de ficheros en el directorio son una implementación de lo indicado en  \texttt{spec2.md}, hecha con la ayuda de un asistente de IA.

Carpeta de código: \texttt{Web-Spec/mvc}

%%-----------------------------------------------------------------------------
\subsection{Primera aplicación Django}
\label{subsec:django-primera}

\textbf{Enunciado:}

Especifica con un fichero \texttt{spec.md} una aplicación para que los usuarios puedan poner notas en un blog, y ver las que han puesto los demás. Crea, a partir de ella, una aplicación Django.
...
\url{https://gitlab.eif.urjc.es/cursosweb/practicas/server/xml-barrapunto}
...
Repositorio de inicio: \url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/videos-canal-youtube}.
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2022-2023/xml-youtube-descarga}
...
\url{https://gitlab.eif.urjc.es/cursosweb/practicas/server/django-cms-barrapunto}
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2023-2024/django-youtube}
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2022-2023/django-youtube}
...
\begin{verbatim}
gitlab-forks.py gitlab.eif.urjc.es 2799 <token>
\end{verbatim}
...
\begin{verbatim}
[
  "https://gitlab.eif.urjc.es/xxx/youtube-descarga.git",
  "https://gitlab.eif.urjc.es/yyy/youtube-descarga.git",
  "https://gitlab.eif.urjc.es/zzz/youtube-descarga.git"
]
\end{verbatim}
...
curl --header "PRIVATE-TOKEN: <token>"
  "https://gitlab.eif.urjc.es/api/v4/projects/2799/forks"
...
%%-------------------------------------------------------------------------
%%-------------------------------------------------------------------------
\subsection{Django cms\_css basico}
...
Repositorio de entrega en GitLab: \\
\url{https://gitlab.eif.urjc.es/cursosweb/2024-2025/django-cms-css-basico}
...
Repositorio de entrega en GitLab: \\
\url{https://gitlab.eif.urjc.es/cursosweb/practicas/server/django-cms-css}
...
Repositorio de entrega en GitLab: \\
\url{https://gitlab.eif.urjc.es/cursosweb/practicas/server/django-cms-css-2}
...
Repositorio de entrega en GitLab: \\
\url{https://gitlab.eif.urjc.es/cursosweb/django-cms-cuadricula}
...
A partir del ejercicio ``Django cms'' o cualquiera de sus derivados que hayas realizado, crea una nueva aplicación de forma que utilice uno de los siguientes ejemplos personalizados con componentes de Bootstrap:

\begin{itemize}
  \item Carousel: https://getbootstrap.com/docs/5.3/examples/carousel/
  \item Jumbotron: https://getbootstrap.com/docs/5.3/examples/jumbotron/
  \item Album: https://getbootstrap.com/docs/5.3/examples/album/
\end{itemize}

Para este ejercicio, si hicieran falta imágenes, se pueden a) utilizar imágenes en Internet (que no se servirán por nuestra instancia Django) o b) incluir imágenes servidas por el propio Django (como ficheros estáticos).

Repositorio de inicio de GitLab: \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/django-cms-bootstrap}
...
%%-------------------------------------------------------------------------
%%-------------------------------------------------------------------------
\subsection{Asistente de IA generativa para PyCharm (Codeium)}
\label{subsec:practicas-pycharm-ia-codeium}

\textbf{Enunciado:}

Instala el plugin de PyCharm ``Codeiun''. Configúralo y pruébalo.
...
%%-------------------------------------------------------------------------
%%-------------------------------------------------------------------------
\subsection{Asistente de IA generativa para PyCharm (Windsurf)}
\label{subsec:practicas-pycharm-ia}

\textbf{Enunciado:}

Instala el plugin de PyCharm ``Windsurf''. Configúralo y pruébalo.

Puedes probar también otros plugins con funcionalidad similar, como Copilot o Kilo.

\textbf{Materiales:}

\begin{itemize}
\item Información sobre el plugin Windsurf: \\
  \url{https://plugins.jetbrains.com/plugin/20540-windsurf-plugin-for-python-js-java-go--}
\item Sitio web de Windsurf: \\
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/calculadora}
...
% \textbf{Repositorio plantilla (para entrega):} \\
% \url{https://gitlab.eif.urjc.es/cursosweb/2022-2023/descarga-documentos-web}
...
El programa principal creará al menos dos objetos de la clase Robot, llamará a varios de sus métodos, para mostrar que todo funciona.
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/descarga-documentos-web-con-modulos}
...
%%------------------------------------------------------------------------
%%------------------------------------------------------------------------
\subsection{Cabeceras de recursos web}
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/urls-aleatorias}
...
%Parte para ello (i.e., haz un \emph{fork}) del siguiente repositorio: \url{https://gitlab.eif.urjc.es/grex/x-serv-14.3-urlsaleatorias}
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/aplicacion-redirectora}
...
Repositorio de inicio: \url{https://gitlab.eif.urjc.es/grex/x-serv-14.5-sumador-simple}
...
Repositorio de inicio: \url{https://gitlab.eif.urjc.es/grex/x-serv-14.7-servurlaleat}
...
Repositorio de inicio: \url{https://gitlab.eif.urjc.es/grex/X-Serv-14.8-Servidor-Aplicaciones-Sumador}
...
Repositorio de inicio: \url{https://gitlab.eif.urjc.es/grex/X-Serv-14.9-ServVariasApps}
...
Repositorio de inicio: \url{https://gitlab.eif.urjc.es/grex/X-Serv-14.10-CuatroAplis}
...
Parte del repositorio en GitLab: \verb|https://gitlab.eif.urjc.es/cursosweb/x-serv-15.4-django-calc|. El proyecto Django se llamará \texttt{project} y la aplicación \texttt{calc}. 
...
% \url{https://gitlab.eif.urjc.es/grex/x-serv-15.5-django-cms}. El repositorio contiene
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2024-2025/django-cms-put}
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2022-2023/django-cms-put}
...
Las aplicación Django ``Admin site'', entre otras, utiliza el middleware CSRF (protección frente a ``Cross Site Request Forgery''), que hay que tener en cuenta, especialmente en los formularios POST. En particular, es importante asegurarse de que se han referenciado el módulo de CSRF en settings.py:
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/django-users}
...
\url{https://gitlab.eif.urjc.es/cursosweb/practicas/server/django-cms-post}.
...
\url{https://gitlab.eif.urjc.es/cursosweb/practicas/server/django-cms-forms}.
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/django-tests-gitlab}
...
%%----------------------------------------------------------------------------
%%----------------------------------------------------------------------------
\subsection{Django cms persistente}
\label{subsec:django-cms-persistente}

\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026-ltaw/django-cms-persistente}

\textbf{Enunciado:}

Partiendo de la especificación del Django CMS no persistente (carpeta Python-Django/django-cms-nopersistente-spec del repositorio de código), realiza una versión persistente del mismo, utilizando la base de datos de Django (modelos) en lugar de un diccionario Python para almacenar los contenidos.

Crea un fichero \texttt{spec.md} con la especificación de la aplicación. Este fichero tendrá que tener las siguientes secciones:

\begin{itemize}
\item \textbf{Requisitos funcionales} (genéricos): descripción general de la funcionalidad de la aplicación.
\item \textbf{Requisitos no funcionales}: restricciones técnicas (framework, nombre de proyecto, nombre de app, etc.).
\item \textbf{Estado} (datos que se mantienen por la aplicación entre invocaciones HTTP): en esta sección se deben especificar las tablas de la base de datos de Django que se van a utilizar (modelos), indicando para cada tabla sus campos y los tipos de cada campo (por ejemplo, \texttt{CharField}, \texttt{TextField}, \texttt{DateTimeField}, etc.). Recuerda que los contenidos ya no se almacenan en un diccionario, sino en tablas de la base de datos.
\item \textbf{Inicialización}: acciones a realizar cuando arranque la aplicación (por ejemplo, migraciones y carga de datos iniciales si los hubiera).
\item \textbf{Recursos}: descripción de todos los recursos que atiende la aplicación, las operaciones (métodos HTTP) para cada uno de ellos, la información de la petición que se atiende para cada uno de ellos, y la información que se enviará en la respuesta correspondiente.
\end{itemize}

Crea también un fichero \texttt{entrega.md} para indicar, brevemente:
...
La funcionalidad básica (primera fase) es la misma que la del CMS no persistente, pero usando modelos de Django (base de datos) en lugar de un diccionario. La página principal listará los recursos disponibles como enlaces, y al acceder a un recurso se mostrará su contenido. Si el recurso no existe, se devolverá un error 404.

En una segunda fase, añade la posibilidad de crear nuevos contenidos y editar los existentes mediante formularios HTML (usando POST). Cuando se acceda a un recurso que no existe, se mostrará un formulario que permita crear el contenido. Si el recurso ya existe, se mostrará su contenido junto con un formulario que permita editarlo.

En una tercera fase (opcional), añade la posibilidad de que los usuarios puedan dejar comentarios en los contenidos.

\textbf{Materiales:}

\begin{itemize}
\item Repositorio con la especificación no persistente: \\
  \url{https://github.com/CursosWeb/Code/tree/master/Python-Django/django-cms-nopersistente-spec}
\end{itemize}
...
Repositorio en GitLab para entregar el ejercicio: \\ 
\url{https://gitlab.eif.urjc.es/grex/X-Serv-15.12-Django-feedexpander}.
...
Realiza las siguientes modificaciones a la aplicación de Django llamada conciertos que encontrarás en \url{https://gitlab.eif.urjc.es/cursosweb/16.14-conciertos}:
...
%%----------------------------------------------------------------------------
%%----------------------------------------------------------------------------
\subsection{Interacción con un LLM (con especificación)}
\label{subsec:django-llm}

\textbf{Enunciado:}

Utiliza los servicios de alguna API proveedora de servicios de LLM para construir una aplicación web que permita interactuar con los LLM.
...
%%----------------------------------------------------------------------------
%%----------------------------------------------------------------------------
\subsection{Autenticación con OAuth (con especificación)}
\label{subsec:django-oauth}

\textbf{Enunciado:}

Desarrolla una aplicación web en Django que permita crearse cuentas autenticándose con OAuth. Los datos sobre las cuentas creadas se almacenarán en la base de datos, usando en lo posible el sistema de usuarios que proporciona Django. La página principal permitirá autenticarse mediante OAuth. Para la autenticación OAuth se usará, por ejemplo, el servicio proporcionado por GitHub.

\textbf{Recursos:}

\begin{itemize}
\item Módulo Django AllAuth: \url{https://allauth.org/}
\item GitHup documentation, Creating an OAuth app: \\
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/contentapp}
...
[Nota: Poster no está actualizado a las últimas versiones de Firefox, mejor usa RESTer, ver ejercicio~\ref{subsec:inst-rester}]
...
\textbf{Repositorio plantilla (para entrega):} \\
\url{https://gitlab.eif.urjc.es/cursosweb/2025-2026/contentpostapp}
...
\textbf{Repositorio de inicio:} \url{https://gitlab.eif.urjc.es/cursosweb/X-Serv-17.4-ContentPostApp}
...
Prueba la aplicación, y mira cómo se comporta analizando su comportamiento con la pestaña ``Red'' de las herramientas de desarrollador del navegador. Prueba también a interaccionar con los recursos que sirve usando curl.
...
%%-----------------------------------------------------------------------------
\subsection{Gestor de contenidos con especificación}
\label{subsec:contenidos-spec}

\textbf{Enunciado:}

Diseñar una aplicación HTTP que sirva para almacenar contenidos ligados a nombres de recursos. La aplicación debe permitir que los usuarios proporcionen nombres de recursos, por ejemplo ``/hola'' y un contenido para almacenar con ellos, que será lo que vaya en el cuerpo (dentro del elemento \texttt{BODY}) de HTML. Por ejemplo, se puede asociar al recurso ``/hola'' el contenido ``Hola qué tal''. Cuando en el futuro la aplicación reciba una petición para ese recurso, devolverá una página HTML que tendrá en su elemento \texttt{BODY} el contenido que se le haya asignado. Los usuarios podrán también cambiar los contenidos. Para poder hacerlo, la página HTML que se devuelva al navegador incluirá, además de lo indicado antes, y también dentro del elemento  \texttt{BODY}, un formulario.
...
\input{exa-saro-2026-05.tex}

\input{exa-ltaw-2026-05.tex}

\input{exa-sat-2025-06.tex}

\input{exa-saro-2025-06.tex}

\input{exa-sat-2025-05.tex}

\input{exa-saro-2025-05.tex}

\input{exa-sat-2024-06.tex}

\input{exa-saro-2024-06.tex}

\input{exa-sat-2024-05.tex}

\input{exa-saro-2024-05.tex}

\input{exa-sat-2023-06-28}

\input{exa-sat-2023-05-15}
...
\input{practica-final-2025-05}

\newpage

\input{practica-final-2024-05}
